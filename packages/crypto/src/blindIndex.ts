...
export async function computeBlindIndex(indexKey: CryptoKey, value: string): Promise<string> {
  const normalized = normalizeForIndex(value);
  const signature = await crypto.subtle.sign("HMAC", indexKey, toArrayBuffer(textToBytes(normalized)));
